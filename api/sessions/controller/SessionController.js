    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        try {
          ...
          sessionResponse = {
            tokenId: sessionData.sessionId,
            sessionLink: "https://class.thestreamboard.com/?q=" + sessionData.sessionId,
          }
          await SessionModel.updateOne({ sessionId: sessionData.sessionId }, { sessionLink: "https://class.thestreamboard.com/?q=" + sessionData.sessionId }).lean();
          console.log("Sending response to user");
          return successResponseWithData(
            res,
            SessionConstants.registrationSuccessMsg,
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        if (!exists) {
          attendence.push({
            user: userProfile._id, writeAccess: false, huddle: "", sessionId: "", loggedIn: true
          });
        }
